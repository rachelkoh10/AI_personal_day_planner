...
app.post('/api/planner/generate', async (req, res) => {
  try {
    ...
    const preferences: UserPreferences = {
      location: body.preferences?.location || 'Central Singapore',
      timeAvailable: body.preferences?.timeAvailable || 'half_day',
      budgetTier: body.preferences?.budgetTier || 'under_50',
      categories: body.preferences?.categories || ['food', 'nature', 'culture'],
      walkingComfort: body.preferences?.walkingComfort || 'some',
      groupType: body.preferences?.groupType || 'solo',
      seniorGentleMode: Boolean(body.preferences?.seniorGentleMode),
      indoorPreference: Boolean(body.preferences?.indoorPreference),
    };
    const promptOverride: string = body.promptOverride || '';
    ...
    let aiSummary = '';
    let aiReasoning = {
      budgetExplanation: `The total estimated cost is S$32, well within your budget allowance.`,
...
`;
        ...
        console.warn('Gemini call notice (using rule-based reasoning):', err);
      }
    }
    ...
    const plan: TripPlan = {
      id: `plan-${Date.now()}`,
      title: aiSummary || (preferences.seniorGentleMode ? 'Gentle Afternoon in Singapore' : `Smart Afternoon in ${preferences.location}`),
    ...
    const fallbackPlaces = SINGAPORE_PLACES.slice(0, 4);
    const fallbackPlan: TripPlan = {
      id: `plan-${Date.now()}`,
      title: 'Smart Afternoon in Central Singapore',
      createdAt: new Date().toLocaleDateString('en-SG', { day: 'numeric', month: 'short', year: 'numeric' }),
      preferences: defaultPrefs,
      items: fallbackPlaces.map((place, idx) => ({
        id: `item-${idx + 1}`,
        startTime: `${2 + idx}:00 PM`,
        endTime: `${3 + idx}:00 PM`,
        activity: place,
        tips: place.seniorFriendly ? 'Seating and step-free paths available.' : 'Great visit spot.',
      })),
      totalCostEstimate: 35,
      totalDurationMinutes: 240,
      walkingLevel: 'Moderate',
      weatherStatus: 'Caution',
      weatherForecastText: 'Passing showers possible.',
      reasoning: {
        budgetExplanation: 'Cost estimated at S$35 within standard budget.',
        timeExplanation: 'Comfortable 4-hour timeline.',
        walkingExplanation: 'Direct transport connections between stops.',
        weatherExplanation: 'Indoor options included.',
        overallSummary: 'Curated Singapore itinerary.',
      },
      sources: MOCK_DATA_SOURCES,
      monitoringActive: true,
    };
      ...
      steps: [
        {
          ...
          timestamp: new Date().toISOString(),
          phase: 'Assembly',
          description: 'Itinerary generated successfully from curated Singapore dataset',
          status: 'success',
        },
      ],
    });
...
app.post('/api/planner/replan', async (req, res) => {
  try {
    ...
    if (!existingPlan || !existingPlan.items) {
      return res.status(400).json({ error: 'Valid existingPlan is required for replanning' });
    }
    ...
    return res.json({ plan: updatedPlan, steps });
  } catch (error: any) {
    console.error('Error in replan:', error);
    return res.status(500).json({ error: error.message || 'Failed to replan itinerary' });
  }
});
